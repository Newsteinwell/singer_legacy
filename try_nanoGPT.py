...
n_layers = 6 
n_embed = 384 
num_heads = 6  # num_heads * head_size = n_embed
dropout = 0.2

# reset some parameters, for getting a smaller model and test some functions
...
if reset_parameters: 
    ...
    n_layers = 4 
    n_embed = 128 
    num_heads = 4  # num_heads * head_size = n_embed
...
print(f"Total parameters of model : {human_readable_count(total_params)}")
...
model_filename = f'./ckpts/model_{dataset_name}_{current_time}.pth'
...
def save_text(text, filename):
    ...
    while True:
        try:
            ...
            filename = f"./results/{filename.split('.')[0]}_{count}.txt"
            count += 1
# Simplest Sharkspeare text generation model: 
generate_text(model, max_new_tokens=10000)


def inference(model_filename, input_text='请给我写一首诗', save_output=True, max_new_tokens=1000):
    model = BigramLanguageModel()
    ...
    print (f'input text is {input_text}, input of model is {decode(input_token_ids)}')
    generated_idx = model.generate(idx=torch.tensor(input_token_ids, dtype=torch.long, device=device).view(1, -1), max_new_tokens=max_new_tokens)
    generated_text = decode(generated_idx[0].tolist())
    print (generated_text)
    if save_output:
        save_text(generated_text, 'infer.txt')    

inference(model_filename=model_filename)